...
while True:
    ...
    ( cnts, _) = cv2.findContours(result, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)
    # cnts = sorted(cnts, key=cv2.contourArea)
    cv2.drawContours(frame, cnts, -1, (0, 0, 0), 1)
    frame_resized = cv2.resize(frame, (int(frame.shape[1] * 500 / frame.shape[0]), 500))
    cv2.imshow("Edge", frame_resized)
